...
def calibrate_gauge(img1, gauge_number, file_type):
    ...
    for i in range(0, interval):
        ...
        cv2.putText(img1, '%s' % (int(i * separation)), (int(p_text[i][0]), int(p_text[i][1])), cv2.FONT_HERSHEY_SIMPLEX,
                    0.3, (0, 0, 0), 1, cv2.LINE_AA)

    cv2.imwrite('gauge-%s-calibration.%s' % (gauge_number, file_type), img1)
    ...
    return img1,min_angle, max_angle, min_value, max_value, units, x, y, r


def get_current_value(img2, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type):
...
img1, min_angle, max_angle, min_value, max_value, units, x, y, r = calibrate_gauge(img, gauge_number, file_type)

# feed an image (or frame) to get the current value, based on the calibration, by default uses same image as calibration
img = cv2.imread('1.jpg')
img2, val = get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type)
