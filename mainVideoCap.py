# СИСТЕМА КОМПЬЮТЕРНОГО ЗРЕНИЯ ДЛЯ ОПРЕДЕЛЕНИЯ ПОКАЗАНИЙ МАНОМЕТРОВ
...
def PressureGaugeCheck(image, min_a, max_a, min_v, max_v):
    # calibrate function
    # ======================================================================================
    ...
    # restricting the search from 35-48% of the possible radii gives fairly good results across different
    # samples.  Remember that these are pixel values which correspond to the possible radii search range.
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 20, np.array([]), 100, 50, int(height * 0.35),
                               int(height * 0.48))
    # average found circles, found it to be more accurate than trying to tune HoughCircles parameters to get
    # just the right one
    a, b, c = circles.shape
    ...
    cv2.circle(image, (x, y), 2, (255, 0, 0), 3, cv2.LINE_AA)  # draw center of circle
    ...
    for i in range(0, interval):
        for j in range(0, 2):
            if j % 2 == 0:
                ...
                p_text[i][j] = x - text_offset_x + 1.2 * r * np.cos((separation) * (
                        i + 9) * 3.14 / 180)  # point for text labels, i+9 rotates the labels by 90 degrees
            else:
                p2[i][j] = y + r * np.sin(separation * i * 3.14 / 180)
                p_text[i][j] = y + text_offset_y + 1.2 * r * np.sin((separation) * (
                        i + 9) * 3.14 / 180)  # point for text labels, i+9 rotates the labels by 90 degrees
...
while cap.isOpened():
    ...
    try:
        ...
        x = "do nothing"
    cv2.imshow('Считыватель показате', img)
